...
function generateContactNotificationEmail(contact: DatabaseContact): EmailTemplate {
  ...
  const html = `
    ...
    <body>
      <div class="header">
        ...
        <p style="margin: 10px 0 0 0;">WebAssist Contact Form</p>
      </div>
      ...
      <div class="footer">
        <p>This email was automatically generated from the WebAssist contact form.</p>
  ...
  const text = `
New Contact Form Submission - WebAssist
...
export async function sendContactNotification(contact: DatabaseContact): Promise<boolean> {
  try {
    ...
    const fromEmail = process.env.FROM_EMAIL || 'noreply@webassist.com';
    const toEmail = process.env.TO_EMAIL || 'contact@webassist.com';
...
export async function sendAutoReply(contact: DatabaseContact): Promise<boolean> {
  try {
    ...
    const subject = 'Thank you for contacting WebAssist';
    ...
    const html = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Thank you for contacting WebAssist</title>
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; }
      ...
      <body>
        ...
        <div class="content">
          ...
          <p>Thank you for reaching out to WebAssist! We've received your inquiry about your <strong>${contact.projectType.toLowerCase()}</strong> project.</p>
          ...
          <p>Best regards,<br>
          <strong>The WebAssist Team</strong></p>
        </div>

        <div class="footer">
          <p>WebAssist - Professional Web Development Services</p>
...
Thank you for reaching out to WebAssist! We've received your inquiry about your ${contact.projectType.toLowerCase()} project.
...
export async function testEmailConfiguration(): Promise<boolean> {
  try {
    ...
    const result = await resend.emails.send({
      from: fromEmail,
      to: toEmail,
      subject: 'WebAssist Email Configuration Test',
      html: '<p>This is a test email to verify your WebAssist email configuration is working correctly.</p>',
      text: 'This is a test email to verify your WebAssist email configuration is working correctly.',
    });
